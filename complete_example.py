...
print(f"   [OK] Created {len(sequences)} training sequences")

# Train predictor
print("\n2. Training MarkovPredictor...")
predictor = MarkovPredictor(order=1, history_size=10)
predictor.fit(sequences)
print(f"   [OK] Trained on {predictor.vocab_size} unique APIs")
...
print(f"   [OK] After 'login', top 3 predictions:")
...
print(f"   [OK] First 5 values (indices): {state[:5]}")

# Evaluate accuracy
print("\n5. Evaluating accuracy...")
evaluator = MarkovEvaluator(predictor)
results = evaluator.evaluate_accuracy(sequences, k_values=[1, 3, 5])
print(f"   [OK] Top-1 Accuracy:  {results['top_1_accuracy']:.1%}")
print(f"   [OK] Top-3 Accuracy:  {results['top_3_accuracy']:.1%}")
print(f"   [OK] Top-5 Accuracy:  {results['top_5_accuracy']:.1%}")
print(f"   [OK] MRR:             {results['mrr']:.3f}")
print(f"   [OK] Coverage:        {results['coverage']:.1%}")
print(f"   [OK] Perplexity:      {results['perplexity']:.2f}")

# Per-endpoint breakdown
print("\n6. Per-endpoint breakdown...")
per_endpoint = evaluator.evaluate_per_endpoint(sequences)
print(f"   [OK] Analyzed {len(per_endpoint)} endpoints")
...
print(f"   [OK] Calibration bins: {len(calibration['bin_centers'])}")
...
try:
    ...
    print(f"   [OK] Transition heatmap saved")
    ...
    print(f"   [OK] Confidence distribution saved")
...
print(f"   [OK] Model saved to data/test/example_predictor.json")
...
print(f"   [OK] Model comparison:")
print(f"\n   Model          Top-1    Top-3    MRR")
for _, row in comparison.iterrows():
    print(f"   {row['model']:12s}   {row['top_1_accuracy']:.1%}    {row['top_3_accuracy']:.1%}   {row['mrr']:.3f}")

print("\n" + "=" * 70)
print("[OK] COMPLETE EXAMPLE FINISHED!")
print("=" * 70)
print("\nAll features demonstrated:")
print("  [OK] Training and predictions")
print("  [OK] State vectors for RL integration")
print("  [OK] Comprehensive evaluation metrics")
print("  [OK] Per-endpoint breakdown")
