...
try:
    ...
    print(f"[OK] Agent created (device: {agent.device})")

    # Test action selection
    state = np.random.randn(60).astype(np.float32)
    action = agent.select_action(state)
    print(f"[OK] Selected action: {action} (epsilon={agent.epsilon:.2f})")
    ...
    print("[OK] Agent saved")

    agent2 = DQNAgent(config)
    agent2.load("test_agent.pt")
    print(f"[OK] Agent loaded (epsilon: {agent2.epsilon:.3f})")
